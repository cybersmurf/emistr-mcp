...
class DatabaseManager:
    ...
    def _serialize_row(row: Dict) -> Dict:
        ...
        for key, value in row.items():
            ...
            # Přidáno pro správnou konverzi desetinných čísel
    ...
    async def get_orders(
        ...
        query += " ORDER BY o.priorita DESC, o.start ASC LIMIT %s"
        params.append(limit)
    ...
    async def get_materials(
        ...
        query = """
            SELECT 
                ...
                sm.name,
                sm.bar_id,
                sr.mnozstvi as stock_quantity,
                sr.min_mnozstvi as min_quantity,
                sr.rezervovano,
                sm.jednotka,
                sm.cena_nakup,
                sk.name as warehouse_name
            FROM sklad_material sm
            LEFT JOIN sklad_material_readdata sr ON sm.id = sr.material_id
            LEFT JOIN sklady_materialu sk ON sr.sklad_id = sk.id
            WHERE 1=1
        """
        ...
        if sklad_id:
            query += " AND sr.sklad_id = %s"
            params.append(sklad_id)
        
        if low_stock_only:
            query += " AND sr.mnozstvi < sr.min_mnozstvi"
        
        query += " ORDER BY sm.name LIMIT %s"
        params.append(limit)
        
        materials = await self.execute_query(query, tuple(params))
    ...
    async def get_operations(
        ...
        if operation_group:
            query += " AND op.group_name = %s"
            params.append(operation_group)
        
        query += " ORDER BY op.name LIMIT %s"
        params.append(limit)
        
        operations = await self.execute_query(query, tuple(params))
    ...
    async def get_machines(
        ...
        query = """
            SELECT 
                ...
                s.active,
                s.group_name,
                ast.stav as current_status,
                ast.order_id as current_order_id,
                ast.worker_id as current_worker_id
            FROM stroje s
            LEFT JOIN aktualni_stav ast ON s.id = ast.stroj_id
            WHERE 1=1
        """
        ...
        if status_filter:
            query += " AND ast.stav = %s"
            params.append(status_filter)
        
        query += " ORDER BY s.name LIMIT %s"
        params.append(limit)
        
        machines = await self.execute_query(query, tuple(params))
    ...
    async def get_production_stats(
        ...
        hours_query = """
            SELECT 
                DATE(rd.datum) as date,
                SUM(rd.real_time) as total_hours,
                COUNT(DISTINCT rd.worker_id) as workers_count,
                COUNT(DISTINCT rd.order_id) as orders_count
            FROM readdata rd
            WHERE rd.datum BETWEEN %s AND %s
            GROUP BY DATE(rd.datum)
            ORDER BY date
        """
